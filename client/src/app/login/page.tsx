  ...
  return (
    <div className="h-screen w-full bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 relative overflow-y-auto lg:overflow-hidden flex flex-col lg:grid lg:grid-cols-12 transition-colors duration-200">
      ...
      <div className="hidden lg:flex lg:col-span-6 xl:col-span-6 p-6 lg:p-8 xl:p-10 flex-col justify-between border-r border-slate-200 dark:border-slate-800/80 relative z-10 bg-slate-100/70 dark:bg-slate-950/60 h-full overflow-hidden transition-colors duration-200">
        ...
        {/* Center CCTV Terminal Mockup with Light & Dark Mode */}
        <div className="my-auto rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 shadow-xl dark:shadow-2xl relative overflow-hidden group max-w-xl text-slate-800 dark:text-slate-100 transition-colors duration-200">
          <div className="flex items-center justify-between pb-2.5 border-b border-slate-200 dark:border-slate-800 text-[11px] font-mono">
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-red-500 animate-ping" />
              <span className="font-bold text-red-500 dark:text-red-400 uppercase tracking-wider">● REC • LIVE</span>
              <span className="text-slate-300 dark:text-slate-600">|</span>
              <span className="text-blue-600 dark:text-blue-300 font-semibold">CAM-01 [TURNIKET-KIRISH]</span>
            </div>
            <span className="text-emerald-700 dark:text-emerald-400 font-bold bg-emerald-50 dark:bg-emerald-950/70 px-2.5 py-0.5 rounded border border-emerald-200 dark:border-emerald-800 text-[10px]">
              60 FPS • 1080p
            </span>
          </div>

          <div className="relative h-40 sm:h-44 my-2.5 rounded-xl bg-slate-100/80 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 flex items-center justify-center overflow-hidden transition-colors duration-200">
            <div
              className="absolute inset-0 opacity-15 pointer-events-none"
              style={{
                backgroundImage: theme === 'dark' 
                  ? `radial-gradient(circle, #38bdf8 1px, transparent 1px)`
                  : `radial-gradient(circle, #0284c7 1.5px, transparent 1.5px)`,
                backgroundSize: '20px 20px',
              }}
            />
            <div className="absolute inset-x-0 h-0.5 bg-gradient-to-r from-transparent via-blue-500 dark:via-cyan-400 to-transparent shadow-[0_0_12px_#38bdf8] animate-pulse" />

            <div className="relative w-28 h-32 sm:w-32 sm:h-36 border-2 border-emerald-500 dark:border-emerald-400/90 bg-emerald-500/5 dark:bg-transparent rounded-2xl flex flex-col justify-between p-2 shadow-[0_0_24px_rgba(16,185,129,0.2)] dark:shadow-[0_0_24px_rgba(52,211,153,0.3)]">
              <div className="flex justify-between">
                <span className="w-3 h-3 border-t-2 border-l-2 border-emerald-600 dark:border-emerald-300" />
                <span className="w-3 h-3 border-t-2 border-r-2 border-emerald-600 dark:border-emerald-300" />
              </div>

              <div className="self-center flex flex-col items-center">
                <div className="w-8 h-8 rounded-full border border-dashed border-emerald-600 dark:border-emerald-400/70 animate-spin" />
                <span className="text-[10px] font-mono font-bold text-emerald-700 dark:text-emerald-300 mt-1">99.4% MATCH</span>
              </div>

              <div className="flex justify-between">
                <span className="w-3 h-3 border-b-2 border-l-2 border-emerald-600 dark:border-emerald-300" />
                <span className="w-3 h-3 border-b-2 border-r-2 border-emerald-600 dark:border-emerald-300" />
              </div>

              <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 whitespace-nowrap bg-emerald-600 text-white font-bold text-[10px] px-2.5 py-0.5 rounded-full shadow-md flex items-center gap-1">
                <span>✓</span>
                <span>{language === 'uz' ? 'ANIQLANDI: O‘QITUVCHI' : 'ИДЕНТИФИЦИРОВАН'}</span>
              </div>
            </div>

            <div className="absolute top-2 left-2 text-[10px] font-mono text-slate-500 dark:text-slate-400 space-y-0.5">
              <div>AI ENGINE: <span className="text-blue-600 dark:text-cyan-300 font-semibold">TinyFace-v2</span></div>
              <div>LATENCY: <span className="text-emerald-600 dark:text-emerald-400 font-semibold">18ms</span></div>
            </div>
            <div className="absolute bottom-2 right-2 text-[10px] font-mono text-slate-500 dark:text-slate-400">
              STATUS: <span className="text-emerald-600 dark:text-emerald-400 font-bold">TURNIKET OCHILDI</span>
            </div>
          </div>

          <div className="grid grid-cols-3 gap-2 pt-2 border-t border-slate-200 dark:border-slate-800 text-center text-[10px] sm:text-[11px]">
            <div className="p-1.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 transition-colors">
              <span className="font-bold text-blue-600 dark:text-blue-400 block">3D-Yuz</span>
              <span className="text-slate-500 dark:text-slate-400 text-[9px]">{language === 'uz' ? '3 rakursli' : '3 ракурса'}</span>
            </div>
            <div className="p-1.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 transition-colors">
              <span className="font-bold text-emerald-600 dark:text-emerald-400 block">0.2 soniya</span>
              <span className="text-slate-500 dark:text-slate-400 text-[9px]">{language === 'uz' ? 'Tezkor qayd' : 'Скорость'}</span>
            </div>
            <div className="p-1.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 transition-colors">
              <span className="font-bold text-indigo-600 dark:text-indigo-400 block">T-13 Excel</span>
              <span className="text-slate-500 dark:text-slate-400 text-[9px]">{language === 'uz' ? 'Avto-tabel' : 'Табель'}</span>
            </div>
          </div>
        </div>
